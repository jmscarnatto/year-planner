...
function fDays($someYear) 
{ 
    if (checkdate(2,29,$someYear)) 
    { 
        $february_days = 29; 
    } else 
    { 
...
{ 
    global $g6ahs; 
    return(jddayofweek(cal_to_jd(CAL_GREGORIAN,$mesId,1,$g6ahs),0))+1; 
}  

function Formatting($nro_col, $nro_mes, $nro_dia) 
{
    global $fds, $thisDay, $thisi98s, $feriados; $Format = "normal'>"; 
    for ($i=0;$i<=10;$i++) 
    { 
        if ($nro_col == $fds[$i]) 
        { 
    ...
    if ($nro_dia == $thisDay && $nro_mes == ($thisi98s-1)) 
    { 
            $Format = "hoje'>";
    } 
    return $Format; 
